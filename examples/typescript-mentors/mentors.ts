import { AiAdmin } from 'AiAdmin-node';
...
async function getSystemPrompt(mentor: Mentor, isLast: boolean, question: string): Promise<string> {
  AiAdmin.setModel(getMentorFileName(mentor));
  const info = await AiAdmin.showModelInfo()
...
async function main() {
  ...
  for await (const mentor of mentors) {
    ...
    AiAdmin.setModel(getMentorFileName(mentor));
    AiAdmin.setSystemPrompt(SystemPrompt);
    let output = '';
    process.stdout.write(`\n${mentor.char}: `);
    for await (const chunk of AiAdmin.streamingGenerate(theConversation + `Continue the conversation as if you were ${mentor.char} on the question "${question}".`)) {
      if (chunk.response) {

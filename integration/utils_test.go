...
func GetTestEndpoint() (*api.Client, string) {
	defaultPort := "11434"
	AiAdminHost := os.Getenv("AiAdmin_HOST")

	scheme, hostport, ok := strings.Cut(AiAdminHost, "://")
	if !ok {
		scheme, hostport = "http", AiAdminHost
	}
	...
	if os.Getenv("AiAdmin_TEST_EXISTING") == "" && port == defaultPort {
		port = FindPort()
	}
...
func startServer(t *testing.T, ctx context.Context, AiAdminHost string) error {
	// Make sure the server has been built
	CLIName, err := filepath.Abs("../AiAdmin")
	if err != nil {
		return err
	}
	...
	if tmp := os.Getenv("AiAdmin_HOST"); tmp != AiAdminHost {
		slog.Info("setting env", "AiAdmin_HOST", AiAdminHost)
		t.Setenv("AiAdmin_HOST", AiAdminHost)
	}

	slog.Info("starting server", "url", AiAdminHost)
	done, err := lifecycle.SpawnServer(ctx, "../AiAdmin")
	if err != nil {
...
func InitServerConnection(ctx context.Context, t *testing.T) (*api.Client, string, func()) {
	...
	if os.Getenv("AiAdmin_TEST_EXISTING") == "" {
		...
		fp, err := os.CreateTemp("", "AiAdmin-server-*.log")
		if err != nil {

...
```javascript
import { AiAdmin } from "@langchain/community/llms/AiAdmin";
...
const answer = await AiAdmin.invoke(`why is the sky blue?`);

console.log(answer);
```

That will get us the same thing as if we ran `AiAdmin run llama3 "why is the sky blue"` in the terminal. But we want to load a document from the web to ask a question against. **Cheerio** is a great library for ingesting a webpage, and **LangChain** uses it in their **CheerioWebBaseLoader**. So let's install **Cheerio** and build that part of the app.
...
const chain = RetrievalQAChain.fromLLM(AiAdmin, retriever);
...
And that is a simple introduction to what you can do with **LangChain** and **AiAdmin.**

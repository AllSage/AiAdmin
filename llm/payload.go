...
var errPayloadMissing = errors.New("expected payloads not included in this build of AiAdmin")
...
func Init() error {
	...
	slog.Debug("Override detection logic by setting AiAdmin_LLM_LIBRARY")

	return nil
}

// binary names may contain an optional variant separated by '_'
// For example, "AiAdmin_rocm_v6" and "AiAdmin_rocm_v5" or "AiAdmin_cpu" and "AiAdmin_cpu_avx2"
...
func getAvailableServers() map[string]string {
	...
	// glob payloadsDir for files that start with AiAdmin_
	pattern := filepath.Join(payloadsDir, "*", "AiAdmin_*")
...
func serversForGpu(info gpu.GpuInfo) []string {
	// glob workDir for files that start with AiAdmin_

...
init_vars
if [ -z "${AiAdmin_SKIP_STATIC_GENERATE}" -o "${AiAdmin_CPU_TARGET}" = "static" ]; then
    # Builds by default, allows skipping, forces build if AiAdmin_CPU_TARGET="static"
...
init_vars
if [ -z "${AiAdmin_SKIP_CPU_GENERATE}" ]; then
    # Users building from source can tune the exact flags we pass to cmake for configuring
    # llama.cpp, and we'll build only 1 CPU variant in that case as the default.
    if [ -n "${AiAdmin_CUSTOM_CPU_DEFS}" ]; then
        init_vars
        echo "AiAdmin_CUSTOM_CPU_DEFS=\"${AiAdmin_CUSTOM_CPU_DEFS}\""
        CMAKE_DEFS="${AiAdmin_CUSTOM_CPU_DEFS} -DBUILD_SHARED_LIBS=off -DCMAKE_POSITION_INDEPENDENT_CODE=on ${CMAKE_DEFS}"
        ...
        if [ -z "${AiAdmin_CPU_TARGET}" -o "${AiAdmin_CPU_TARGET}" = "cpu" ]; then
            #
        ...
        if [ "${ARCH}" == "x86_64" ]; then
            ...
            #
            if [ -z "${AiAdmin_CPU_TARGET}" -o "${AiAdmin_CPU_TARGET}" = "cpu_avx" ]; then
                #
            ...
            if [ -z "${AiAdmin_CPU_TARGET}" -o "${AiAdmin_CPU_TARGET}" = "cpu_avx2" ]; then
                #
...
if [ -z "${AiAdmin_SKIP_CUDA_GENERATE}" -a -d "${CUDA_LIB_DIR}" ]; then
    ...
    if [ -n "${AiAdmin_CUSTOM_CUDA_DEFS}" ]; then
        echo "AiAdmin_CUSTOM_CUDA_DEFS=\"${AiAdmin_CUSTOM_CUDA_DEFS}\""
        CMAKE_CUDA_DEFS="-DGGML_CUDA=on -DCMAKE_CUDA_ARCHITECTURES=${CMAKE_CUDA_ARCHITECTURES} ${AiAdmin_CUSTOM_CUDA_DEFS}"
    ...
    DEPS="$(ldd ${BUILD_DIR}/bin/AiAdmin_llama_server )"
...
if [ -z "${AiAdmin_SKIP_ONEAPI_GENERATE}" -a -d "${ONEAPI_ROOT}" ]; then
    ...
    for dep in $(ldd "${BUILD_DIR}/bin/AiAdmin_llama_server" | grep "=>" | cut -f2 -d= | cut -f2 -d' ' | grep -e sycl -e mkl -e tbb); do
...
if [ -z "${AiAdmin_SKIP_ROCM_GENERATE}" -a -d "${ROCM_PATH}" ]; then
    ...
    # Users building from source can tune the exact flags we pass to cmake for configuring llama.cpp
    if [ -n "${AiAdmin_CUSTOM_ROCM_DEFS}" ]; then
        echo "AiAdmin_CUSTOM_ROCM_DEFS=\"${AiAdmin_CUSTOM_ROCM_DEFS}\""
        CMAKE_DEFS="${CMAKE_DEFS} ${AiAdmin_CUSTOM_ROCM_DEFS}"
    ...
    for dep in $(ldd "${BUILD_DIR}/bin/AiAdmin_llama_server" | grep "=>" | cut -f2 -d= | cut -f2 -d' ' | grep -e rocm -e amdgpu -e libtinfo ); do

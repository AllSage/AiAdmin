...
sign() {
    if [ -n "$APPLE_IDENTITY" ]; then
        codesign -f --timestamp --deep --options=runtime --sign "$APPLE_IDENTITY" --identifier ai.AiAdmin.AiAdmin $1
    fi
}
...
"amd64")
    ...
    if [ -z "$AiAdmin_SKIP_CPU_GENERATE" ]; then
        ...
        build
        sign ${BUILD_DIR}/bin/AiAdmin_llama_server
        compress
...
"arm64")
    ...
    if [ -z "$AiAdmin_SKIP_METAL_GENERATE" ]; then
        init_vars

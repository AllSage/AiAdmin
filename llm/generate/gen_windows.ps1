...
function init_vars {
    ...
    if (!$script:cmakeTargets) {
        $script:cmakeTargets = @("AiAdmin_llama_server")
    }
    $script:cmakeDefs = @(
    ...
    $script:DIST_BASE = "${script:SRC_DIR}\dist\windows-${script:ARCH}\AiAdmin_runners"
...
function apply_patches {
    ...
    if (!(Select-String -Path "${script:llamacppDir}/CMakeLists.txt" -Pattern 'AiAdmin')) {
        Add-Content -Path "${script:llamacppDir}/CMakeLists.txt" -Value 'add_subdirectory(../ext_server ext_server) # AiAdmin'
    }
...
function build_static() {
    if ((-not "${env:AiAdmin_SKIP_STATIC_GENERATE}") -and ((-not "${env:AiAdmin_CPU_TARGET}") -or ("${env:AiAdmin_CPU_TARGET}" -eq "static"))) {
...
function build_cpu($gen_arch) {
    if ((-not "${env:AiAdmin_SKIP_CPU_GENERATE}" ) -and ((-not "${env:AiAdmin_CPU_TARGET}") -or ("${env:AiAdmin_CPU_TARGET}" -eq "cpu"))) {
...
function build_cpu_avx() {
    if ((-not "${env:AiAdmin_SKIP_CPU_GENERATE}" ) -and ((-not "${env:AiAdmin_CPU_TARGET}") -or ("${env:AiAdmin_CPU_TARGET}" -eq "cpu_avx"))) {
        init_vars
...
function build_cpu_avx2() {
    if ((-not "${env:AiAdmin_SKIP_CPU_GENERATE}" ) -and ((-not "${env:AiAdmin_CPU_TARGET}") -or ("${env:AiAdmin_CPU_TARGET}" -eq "cpu_avx2"))) {
        init_vars
...
function build_cuda() {
    if ((-not "${env:AiAdmin_SKIP_CUDA_GENERATE}") -and ("${script:CUDA_LIB_DIR}")) {
        ...
        if ($null -ne $env:AiAdmin_CUSTOM_CUDA_DEFS) {
            write-host "AiAdmin_CUSTOM_CUDA_DEFS=`"${env:AiAdmin_CUSTOM_CUDA_DEFS}`""
            $script:cmakeDefs +=@("${env:AiAdmin_CUSTOM_CUDA_DEFS}")
...
function build_oneapi() {
  if ((-not "${env:AiAdmin_SKIP_ONEAPI_GENERATE}") -and ("${env:ONEAPI_ROOT}"))  {
    # Get oneAPI version
    ...
    if ($null -ne $script:DUMPBIN) {
      & "$script:DUMPBIN" /dependents "${script:buildDir}/bin/AiAdmin_llama_server.exe" | Select-String ".dll"
    }
    sign
    install
...
function build_rocm() {
    if ((-not "${env:AiAdmin_SKIP_ROCM_GENERATE}") -and ("${env:HIP_PATH}")) {
        ...
        $env:LIB=""
        if ($null -ne $env:AiAdmin_CUSTOM_ROCM_DEFS) {
            write-host "AiAdmin_CUSTOM_ROCM_DEFS=`"${env:AiAdmin_CUSTOM_ROCM_DEFS}`""
            $script:cmakeDefs += @("${env:AiAdmin_CUSTOM_ROCM_DEFS}")

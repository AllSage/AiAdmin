...
func NewLlamaServer(gpus gpu.GpuInfoList, model string, ggml *GGML, adapters, projectors []string, opts api.Options, numParallel int) (LlamaServer, error) {
	...
	if len(adapters) > 1 {
		return nil, errors.New("AiAdmin supports only one lora adapter, but multiple were provided")
	}
	...
	if demandLib != "" {
		...
		if serverPath == "" {
			slog.Info(fmt.Sprintf("Invalid AiAdmin_LLM_LIBRARY %s - not found", demandLib))
		} else {
			slog.Info("user override", "AiAdmin_LLM_LIBRARY", demandLib, "path", serverPath)
	...
	for i := range len(servers) {
		...
		server := filepath.Join(dir, "AiAdmin_llama_server")
		if runtime.GOOS == "windows" {
			server += ".exe"
		}
		...
		if err = s.cmd.Start(); err != nil {
			...
			if errors.Is(err, os.ErrPermission) {
				finalErr = fmt.Errorf("unable to start server %w.  %s may have noexec set.  Set AiAdmin_TMPDIR for server to a writable executable directory", err, dir)
				continue
			}
			msg := ""
		...
		go func() {
			...
			if err != nil && s.status != nil && s.status.LastErrMsg != "" {
				...
				if strings.Contains(s.status.LastErrMsg, "unknown model") {
					s.status.LastErrMsg = "this model is not supported by your version of AiAdmin. You may need to upgrade"
				}

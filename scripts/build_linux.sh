...
for TARGETARCH in ${BUILD_ARCH}; do
    docker build \
        ...
        --build-arg=AiAdmin_CUSTOM_CPU_DEFS \
    ...
    docker cp builder-$TARGETARCH:/go/src/github.com/AllSage/AiAdmin/AiAdmin ./dist/AiAdmin-linux-$TARGETARCH

    if [ "$TARGETARCH" = "amd64" ]; then
        docker cp builder-$TARGETARCH:/go/src/github.com/AllSage/AiAdmin/dist/deps/ ./dist/
    fi

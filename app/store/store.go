...
func writeStore(storeFilename string) {
	AiAdminDir := filepath.Dir(storeFilename)
	_, err := os.Stat(AiAdminDir)
	if errors.Is(err, os.ErrNotExist) {
		if err := os.MkdirAll(AiAdminDir, 0o755); err != nil {
			slog.Error(fmt.Sprintf("create AiAdmin dir %s: %v", AiAdminDir, err))
			return
		}
	}

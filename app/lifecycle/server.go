...
func start(ctx context.Context, command string) (*exec.Cmd, error) {
	...
	if err != nil {
		if !errors.Is(err, os.ErrNotExist) {
			return nil, fmt.Errorf("stat AiAdmin server log dir %s: %v", logDir, err)
		}

		if err := os.MkdirAll(logDir, 0o755); err != nil {
			return nil, fmt.Errorf("create AiAdmin server log dir %s: %v", logDir, err)
		}
	}
	...
	if cmd.Process != nil {
		slog.Info(fmt.Sprintf("started AiAdmin server with pid %d", cmd.Process.Pid))
	}
	slog.Info(fmt.Sprintf("AiAdmin server logs %s", ServerLogFile))

...
const logger = winston.createLogger({
  transports: [
    ...
    new winston.transports.File({
      filename: path.join(app.getPath('home'), '.AiAdmin', 'logs', 'server.log'),
...
function updateTray() {
  ...
  const menu = Menu.buildFromTemplate([
    ...
    { role: 'quit', label: 'Quit AiAdmin', accelerator: 'Command+Q' },
  ])
  ...
  tray.setToolTip(updateAvailable ? 'An update is available' : 'AiAdmin')
...
function server() {
  const binary = app.isPackaged
    ? path.join(process.resourcesPath, 'AiAdmin')
    : path.resolve(process.cwd(), '..', 'AiAdmin')
...
const updateURL = `https://AiAdmin.com/api/update?os=${process.platform}&arch=${
  process.arch
    ...
    if (app.isPackaged) {
      if (!app.isInApplicationsFolder()) {
        const chosen = dialog.showMessageBoxSync({
          ...
          message: 'AiAdmin works best when run from the Applications directory.',
          defaultId: 0,
          cancelId: 1,
        })
              ...
              conflictHandler: conflictType => {
                if (conflictType === 'existsAndRunning') {
                  dialog.showMessageBoxSync({
                    ...
                    detail:
                      'Another version of AiAdmin is currently running from your Applications directory. Close it first and try again.',
                  })
                }
                return true
              },

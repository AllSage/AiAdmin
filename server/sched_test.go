...
import (
	...
	"github.com/AllSage/AiAdmin/gpu"
	"github.com/AllSage/AiAdmin/llm"
	"github.com/stretchr/testify/require"
)

func init() {
	os.Setenv("AiAdmin_DEBUG", "1")
...
func TestRequestsSameModelSameRequest(t *testing.T) {
	...
	s.getGpuFn = getGpuFn
	s.getCpuFn = getCpuFn
	a := newScenarioRequest(t, ctx, "AiAdmin-model-1", 10, &api.Duration{Duration: 5 * time.Millisecond})
	b := newScenarioRequest(t, ctx, "AiAdmin-model-1", 11, &api.Duration{Duration: 0})
...
func TestRequestsSimpleReloadSameModel(t *testing.T) {
	...
	s.getCpuFn = getCpuFn
	a := newScenarioRequest(t, ctx, "AiAdmin-model-1", 10, &api.Duration{Duration: 5 * time.Millisecond})
	b := newScenarioRequest(t, ctx, "AiAdmin-model-1", 20, &api.Duration{Duration: 5 * time.Millisecond})
...
func TestRequestsMultipleLoadedModels(t *testing.T) {
	...
	a := newScenarioRequest(t, ctx, "AiAdmin-model-3a", 1*format.GigaByte, nil)
	b := newScenarioRequest(t, ctx, "AiAdmin-model-3b", 24*format.GigaByte, nil)
	c := newScenarioRequest(t, ctx, "AiAdmin-model-4a", 30, nil)
	c.req.opts.NumGPU = 0                                       // CPU load, will be allowed
	d := newScenarioRequest(t, ctx, "AiAdmin-model-3c", 30, nil) // Needs prior unloaded
...
func TestGetRunner(t *testing.T) {
	...
	a := newScenarioRequest(t, ctx, "AiAdmin-model-1a", 10, &api.Duration{Duration: 2 * time.Millisecond})
	b := newScenarioRequest(t, ctx, "AiAdmin-model-1b", 10, &api.Duration{Duration: 2 * time.Millisecond})
	c := newScenarioRequest(t, ctx, "AiAdmin-model-1c", 10, &api.Duration{Duration: 2 * time.Millisecond})
...
func TestPrematureExpired(t *testing.T) {
	...
	scenario1a := newScenarioRequest(t, ctx, "AiAdmin-model-1a", 10, nil)
	s := InitScheduler(ctx)
...
func TestAlreadyCanceled(t *testing.T) {
	...
	done2()
	scenario1a := newScenarioRequest(t, dctx, "AiAdmin-model-1", 10, &api.Duration{Duration: 0})
	s := InitScheduler(ctx)

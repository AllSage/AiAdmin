...
import (
	...
	"github.com/AllSage/AiAdmin/convert"
...
func parseFromModel(ctx context.Context, name model.Name, fn func(api.ProgressResponse)) (layers []*layerGGML, err error) {
	...
	for _, layer := range m.Layers {
		...
		case "application/vnd.AiAdmin.image.model",
			"application/vnd.AiAdmin.image.projector",
			"application/vnd.AiAdmin.image.adapter":
...
func parseFromZipFile(_ context.Context, file *os.File, digest string, fn func(api.ProgressResponse)) (layers []*layerGGML, err error) {
	...
	// e.g. NewLayer(arch.Reader(), "application/vnd.AiAdmin.image.model")
	...
	layer, err := NewLayer(temp, "application/vnd.AiAdmin.image.model")
	if err != nil {
		return nil, err
	}
...
func parseFromFile(ctx context.Context, file *os.File, digest string, fn func(api.ProgressResponse)) (layers []*layerGGML, err error) {
	...
	for offset < stat.Size() {
		...
		mediatype := "application/vnd.AiAdmin.image.model"
		if ggml.Name() == "ggla" {
			mediatype = "application/vnd.AiAdmin.image.adapter"
		} else if ggml.KV().Architecture() == "clip" {
			mediatype = "application/vnd.AiAdmin.image.projector"
		}
	...
	for _, layer := range layers {
		if s := layer.GGML.KV().ChatTemplate(); s != "" {
			if t, err := template.Named(s); err != nil {
				...
				tmpl, err := NewLayer(t.Reader(), "application/vnd.AiAdmin.image.template")
				if err != nil {
					return nil, err
				}

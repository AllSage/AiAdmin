...
import (
	...
	"github.com/AllSage/AiAdmin/llm"
	"github.com/AllSage/AiAdmin/parser"
	"github.com/AllSage/AiAdmin/template"
...
func GetModel(name string) (*Model, error) {
	...
	for _, layer := range manifest.Layers {
		...
		case "application/vnd.AiAdmin.image.model":
			model.ModelPath = filename
			model.ParentModel = layer.From
		case "application/vnd.AiAdmin.image.embed":
			// Deprecated in versions  > 0.1.2
			// TODO: remove this warning in a future version
			slog.Info("WARNING: model contains embeddings, but embeddings in modelfiles have been deprecated and will be ignored.")
		case "application/vnd.AiAdmin.image.adapter":
			model.AdapterPaths = append(model.AdapterPaths, filename)
		case "application/vnd.AiAdmin.image.projector":
			model.ProjectorPaths = append(model.ProjectorPaths, filename)
		case "application/vnd.AiAdmin.image.prompt",
			"application/vnd.AiAdmin.image.template":
		...
		case "application/vnd.AiAdmin.image.system":
			bts, err := os.ReadFile(filename)
			if err != nil {
				return nil, err
			}
		...
		case "application/vnd.AiAdmin.image.params":
		...
		case "application/vnd.AiAdmin.image.messages":
		...
		case "application/vnd.AiAdmin.image.license":
			bts, err := os.ReadFile(filename)
			if err != nil {
				return nil, err
			}
...
func CreateModel(ctx context.Context, name model.Name, modelFileDir, quantization string, modelfile *parser.File, fn func(resp api.ProgressResponse)) (err error) {
	...
	for _, c := range modelfile.Commands {
		mediatype := fmt.Sprintf("application/vnd.AiAdmin.image.%s", c.Name)
		...
		case "model", "adapter":
			...
			for _, baseLayer := range baseLayers {
				if quantization != "" &&
					baseLayer.MediaType == "application/vnd.AiAdmin.image.model" &&
					baseLayer.GGML != nil &&
	...
	layers = slices.DeleteFunc(layers, func(layer *Layer) bool {
		switch layer.MediaType {
		case "application/vnd.AiAdmin.image.message":
	...
	if len(messages) > 0 {
		...
		layer, err := NewLayer(&b, "application/vnd.AiAdmin.image.messages")
		if err != nil {
			return err
		}
	...
	if len(parameters) > 0 {
		...
		layer, err := NewLayer(&b, "application/vnd.AiAdmin.image.params")
		if err != nil {
			return err
		}
...
func makeRequestWithRetry(ctx context.Context, method string, requestURL *url.URL, headers http.Header, body io.ReadSeeker, regOpts *registryOptions) (*http.Response, error) {
	...
	if anonymous {
		...
		return nil, &errtypes.UnknownAiAdminKey{Key: pubKey}
	}
...
func makeRequest(ctx context.Context, method string, requestURL *url.URL, headers http.Header, body io.Reader, regOpts *registryOptions) (*http.Response, error) {
	...
	req.Header.Set("User-Agent", fmt.Sprintf("AiAdmin/%s (%s %s) Go/%s", version.Version, runtime.GOARCH, runtime.GOOS, runtime.Version()))

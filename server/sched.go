...
func (s *Scheduler) processPending(ctx context.Context) {
	for {
		...
		case pending := <-s.pendingReqCh:
			...
			// see https://github.com/AllSage/AiAdmin/issues/4165
				...
				if runner != nil {
					...
					if envconfig.MaxRunners <= 0 {
						...
						if allReliable {
							...
							slog.Debug("updating default concurrency", "AiAdmin_MAX_LOADED_MODELS", envconfig.MaxRunners, "gpu_count", len(gpus))
						} else {
...
func (s *Scheduler) load(req *LlmRequest, ggml *llm.GGML, gpus gpu.GpuInfoList, numParallel int) {
	...
	if err != nil {
		...
		if errors.Is(llm.ErrUnsupportedFormat, err) || strings.Contains(err.Error(), "failed to load model") {
			err = fmt.Errorf("%v: this model may be incompatible with your version of AiAdmin. If you previously pulled this model, try updating it by running `AiAdmin pull %s`", err, req.model.ShortName)
		}

...
func TestParseNameParts(t *testing.T) {
	cases := []struct {
		...
		{
			in: "registry.AiAdmin.ai/library/dolphin-mistral:7b-v2.6-dpo-laser-q6_K",
			want: Name{
				Host:      "registry.AiAdmin.ai",
				Namespace: "library",
				Model:     "dolphin-mistral",
				Tag:       "7b-v2.6-dpo-laser-q6_K",
			},
			wantFilepath: filepath.Join("registry.AiAdmin.ai", "library", "dolphin-mistral", "7b-v2.6-dpo-laser-q6_K"),
		},
		{
		...
		},
		{
			...
			wantFilepath: filepath.Join("registry.AiAdmin.ai", "namespace", "model", "latest"),
		},
		{
			in: "model",
			want: Name{
			...
			wantFilepath: filepath.Join("registry.AiAdmin.ai", "library", "model", "latest"),
		},
		{
			in: "h/nn/mm:t",
			want: Name{
...
func TestNameparseNameDefault(t *testing.T) {
	...
	n := ParseName(name)
	got := n.String()
	want := "registry.AiAdmin.ai/library/xx:latest"
	if got != want {
...
func TestDisplayShortest(t *testing.T) {
	cases := map[string]string{
		"registry.AiAdmin.ai/library/model:latest": "model:latest",
		"registry.AiAdmin.ai/library/model:tag":    "model:tag",
		"registry.AiAdmin.ai/namespace/model:tag":  "namespace/model:tag",

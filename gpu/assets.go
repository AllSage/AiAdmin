...
func PayloadsDir() (string, error) {
	...
	if payloadsDir == "" {
		...
		if tmpDir == "" {
			tmpDir, err = os.MkdirTemp("", "AiAdmin")
			if err != nil {
		...
		pidFilePath := filepath.Join(tmpDir, "AiAdmin.pid")
		...
		// This will typically look like /tmp/AiAdmin3208993108/runners on linux
...
func cleanupTmpDirs() {
	dirs, err := filepath.Glob(filepath.Join(os.TempDir(), "AiAdmin*"))
	if err != nil {
		return
	}
	for _, d := range dirs {
		...
		raw, err := os.ReadFile(filepath.Join(d, "AiAdmin.pid"))
		if err != nil {
			slog.Warn("failed to read AiAdmin.pid", "path", d, "error", err)
		...
		if err == nil && !errors.Is(proc.Signal(syscall.Signal(0)), os.ErrProcessDone) {
			slog.Warn("found running AiAdmin", "pid", pid, "path", d)
			// Another running AiAdmin, ignore this tmpdir
			continue
		}

...
func AMDGetGPUInfo() []RocmGPUInfo {
	...
	if err != nil {
		...
		slog.Warn("AiAdmin recommends running the https://www.amd.com/en/support/linux-drivers", "error", err)
	}
	...
	for _, match := range matches {
		...
		if gpuID < 0 {
			slog.Error("unexpected amdgpu sysfs data resulted in negative GPU ID, please set AiAdmin_DEBUG=1 and report an issue")
			return nil
		}
		...
		if gfxOverride == "" {
			...
			if !slices.Contains[[]string, string](supported, gfx) {
				...
				slog.Warn("See https://github.com/AllSage/AiAdmin/blob/main/docs/gpu.md#overrides for HSA_OVERRIDE_GFX_VERSION usage")
				continue
			} else {
...
func AMDValidateLibDir() (string, error) {
	...
	// Well known AiAdmin installer path
	installedRocmDir := "/usr/share/AiAdmin/lib/rocm"
	if rocmLibUsable(installedRocmDir) {
		return installedRocmDir, nil
	}

	// If we still haven't found a usable rocm, the user will have to install it on their own
	slog.Warn("amdgpu detected, but no compatible rocm library found.  Either install rocm v6, or follow manual install instructions at https://github.com/AllSage/AiAdmin/blob/main/docs/linux.md#manual-install")
